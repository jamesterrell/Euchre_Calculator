...
class PlayRound:
    ...
    def play_round(self, bit: int):
        ...
        contains_target = np.any(
            np.all(all_possible_tricks == self.hands[self.winner][self.card_play], axis=-1), axis=1
        )
        stump = all_possible_tricks[contains_target]
        stump = filter_branch_by_hand(stump, self.hands[(self.winner+1)%4], (self.winner+1)%4, self.hands[self.winner][self.card_play])
        stump = filter_branch_by_hand(stump, self.hands[(self.winner+2)%4], (self.winner+2)%4, self.hands[self.winner][self.card_play])
        stump = filter_branch_by_hand(stump, self.hands[(self.winner+3)%4], (self.winner+3)%4, self.hands[self.winner][self.card_play])
